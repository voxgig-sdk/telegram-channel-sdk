...
// TelegramChannel SDK GetChannelInfo entity
...
class GetChannelInfoEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new GetChannelInfoEntity($this->_client, $opts);
    }

    /**
     * @param GetChannelInfo|array $args GetChannelInfo data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = TelegramChannelHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return GetChannelInfo|array The current GetChannelInfo data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of GetChannelInfo fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = TelegramChannelHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of GetChannelInfo fields).
     */
     ...
     *
     * @param GetChannelInfoLoadMatch|array|null $reqmatch Match criteria (id/query
     *   fields) as an assoc-array; a typed GetChannelInfoLoadMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return GetChannelInfo|array The loaded GetChannelInfo as an assoc-array at the
     *   SDK boundary; throws TelegramChannelError on failure (item-5 convention).
     */
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                ...
                if ($ctx->result->resdata) {
                    $this->_data = TelegramChannelHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
