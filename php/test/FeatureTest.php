...
// TelegramChannel SDK feature test
//
...
class FtCtrl extends TelegramChannelControl
{
...
class FtClient
{
    ...
    private TelegramChannelUtility $utility;

    public function __construct(TelegramChannelUtility $utility, array $options)
    {
    ...
    public function get_utility(): TelegramChannelUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (TelegramChannelContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public function __construct(
        ...
        $utility = new TelegramChannelUtility();
        $this->utility = $utility;

        $utility->fetcher = $server ?? function (TelegramChannelContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = TelegramChannelConfig::make_feature($name);
    ...
    private static function buildUrl(TelegramChannelSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof TelegramChannelError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new TelegramChannelResponse($fetched);
        }
    ...
    private function fail(TelegramChannelContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (TelegramChannelContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
