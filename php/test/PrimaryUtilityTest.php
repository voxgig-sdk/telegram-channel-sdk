...
// TelegramChannel SDK primary utility test
...
class PrimaryUtilityTest extends TestCase
{
    ...
    private static function make_ctx_from_map(
        ?array $ctxmap,
        TelegramChannelSDK $client,
        TelegramChannelUtility $utility
    ): TelegramChannelContext {
        if ($ctxmap === null) {
            $ctxmap = [];
        }

        $ctx = new TelegramChannelContext($ctxmap, null);
    ...
    private static function fixctx(TelegramChannelContext $ctx, TelegramChannelSDK $client): void
    {
    ...
    private static function err_from_map(?array $m): ?TelegramChannelError
    {
        if ($m === null) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new TelegramChannelError($code, $msg);
    }
    ...
    public function test_make_error_no_throw(): void
    {
        ...
        $ctx->result = new TelegramChannelResult([
            'ok' => false,
            'resdata' => ['id' => 'safe01'],
        ]);
    ...
    public function test_feature_add_basic(): void
    {
        ...
        $feature = new TelegramChannelBaseFeature();
    ...
    {
        $hook_client = TelegramChannelSDK::test(null, null);
        $hook_utility = $hook_client->get_utility();
        $ctx = self::make_test_ctx($hook_client, $hook_utility);

        $called = false;
        $hook_feature = new class extends TelegramChannelBaseFeature {
            public $hook_fn = null;
            public function TestHook(TelegramChannelContext $ctx): void
            {
    ...
    {
        $init_client = TelegramChannelSDK::test(null, null);
        ...
        $init_called = false;
        $feature = new class extends TelegramChannelBaseFeature {
            public $init_fn = null;
    ...
    {
        $calls = [];
        $live_client = new TelegramChannelSDK([
            'system' => [
    ...
    {
        $blocked_client = new TelegramChannelSDK([
            'system' => [
    ...
    public function test_make_fetch_def_basic(): void
    {
        ...
        $utility = $client->get_utility();
        $ctx = self::make_test_full_ctx($client, $utility);
        $ctx->spec = new TelegramChannelSpec([
    ...
    public function test_make_result_no_result(): void
    {
        ...
        $utility = $client->get_utility();
        $ctx = self::make_test_full_ctx($client, $utility);
        $ctx->spec = new TelegramChannelSpec(['step' => 'start']);
        $ctx->result = null;
    ...
    public function test_make_spec_basic(): void
    {
        ...
        $spec_client = TelegramChannelSDK::test(null, $setup_opts);
    ...
    public function test_operator_basic(): void
    {
        ...
        $this->runset(self::get_spec($primary, 'operator', 'basic'), function (array $entry) {
            $in = $entry['in'] ?? [];
            $op = new TelegramChannelOperation($in);
            return [[
                'entity' => $op->entity,
                'name' => $op->name,
                'input' => $op->input,
    ...
    public function test_prepare_auth_basic(): void
    {
        ...
        $auth_client = TelegramChannelSDK::test(null, $setup_opts);
    ...
    public function test_result_basic_basic(): void
    {
        ...
        $this->runset(self::get_spec($primary, 'resultBasic', 'basic'), function (array $entry) use ($client, $utility) {
            ...
            if ($result->err !== null) {
                $err_msg = ($result->err instanceof TelegramChannelError) ? $result->err->msg : (string)$result->err;
