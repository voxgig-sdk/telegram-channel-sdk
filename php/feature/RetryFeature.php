...
// TelegramChannel SDK retry feature
...
class TelegramChannelRetryFeature extends TelegramChannelBaseFeature
{
    ...
    public function init(TelegramChannelContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (TelegramChannelContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(TelegramChannelContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(TelegramChannelContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
