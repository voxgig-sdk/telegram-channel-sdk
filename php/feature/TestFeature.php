...
// TelegramChannel SDK test feature

require_once __DIR__ . '/BaseFeature.php';
require_once __DIR__ . '/../utility/Param.php';

class TelegramChannelTestFeature extends TelegramChannelBaseFeature
{
    ...
    public function init(TelegramChannelContext $ctx, array $options): void
    {
        ...
        $test_fetcher = function (TelegramChannelContext $fctx, string $_fullurl, array $_fetchdef) use ($entity): array {
            ...
            if ($op->name === 'load') {
                ...
                $id = TelegramChannelParam::call($fctx, 'id');
    ...
    public function makeNetsim(array $net, callable $inner): callable
    {
        ...
        return function (TelegramChannelContext $fctx, string $url, array $fetchdef) use ($net, $inner, $pick_latency, $sleep): array {
            $this->_netcalls++;
            $call = $this->_netcalls;
     ...
     */
    public function buildArgs(TelegramChannelContext $ctx, $op, $args): array
    {
        ...
        foreach ($keys as $k) {
            ...
            if ($is_id || $in_reqd) {
                $v = TelegramChannelParam::call($ctx, $k);
