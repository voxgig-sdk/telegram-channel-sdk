// Command TelegramChannel-cli is a CLI + REPL for the TelegramChannel SDK,
...
const prompt = "telegram-channel"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "get_channel_info"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      TELEGRAM_CHANNEL_APIKEY (api key), TELEGRAM_CHANNEL_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: TELEGRAM_CHANNEL_APIKEY carries the API key and
	// TELEGRAM_CHANNEL_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("TELEGRAM_CHANNEL_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("TELEGRAM_CHANNEL_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewTelegramChannelSDK(opts)
