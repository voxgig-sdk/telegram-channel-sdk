...
AQL-driven command-line client **and** interactive REPL for the TelegramChannel
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/telegram-channel-cli)
make build

# 2. See usage (words, entities, env vars)
./telegram-channel-cli --help
...
./telegram-channel-cli load 1 get_channel_info            # {id:1} shorthand
./telegram-channel-cli load '{id:1}' get_channel_info       # explicit match map

# 5. Override the API base URL for a single call
TELEGRAM_CHANNEL_BASE=https://api.example.com ./telegram-channel-cli load 1 get_channel_info

# 6. No arguments -> interactive REPL
./telegram-channel-cli
telegram-channel> load 1 get_channel_info
telegram-channel> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/telegram-channel-cli
   ```
...
3. **Run a query.** Evaluate an AQL expression against the API (or run with no
   ...
   ```sh
   ./dist/*/telegram-channel-cli load 1 get_channel_info
   ```
...
```sh
./telegram-channel-cli load 1 get_channel_info          # scalar shorthand for {id:1}
./telegram-channel-cli load '{id:1}' get_channel_info     # explicit match map
```
...
Run with no arguments to open a REPL (prompt `telegram-channel>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an AQL atom (e.g. `get_channel_info`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/telegram-channel-cli`. |
...
(`./telegram-channel-cli <expr>`) and interactively (the REPL), and expressions compose the
