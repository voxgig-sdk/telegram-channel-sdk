...
func envOverride(m map[string]any) map[string]any {
	if os.Getenv("TELEGRAMCHANNEL_TEST_LIVE") == "TRUE" ||
		os.Getenv("TELEGRAMCHANNEL_TEST_OVERRIDE") == "TRUE" {
		for key := range m {
	...
	if explain := os.Getenv("TELEGRAMCHANNEL_TEST_EXPLAIN"); explain != "" {
...
func makeCtxFromMap(ctxmap map[string]any, client *sdk.TelegramChannelSDK, utility *sdk.Utility) *sdk.Context {
	if ctxmap == nil {
	...
	if resMap, ok := ctxmap["result"].(map[string]any); ok {
		...
		if errMap, ok := resMap["err"].(map[string]any); ok {
			if msg, ok := errMap["message"].(string); ok {
				ctx.Result.Err = &sdk.TelegramChannelError{Msg: msg}
			}
		}
	}
...
func fixctx(ctx *sdk.Context, client *sdk.TelegramChannelSDK) {
...
func errFromMap(m map[string]any) error {
	...
	return &sdk.TelegramChannelError{Msg: msg, Code: code}
}
