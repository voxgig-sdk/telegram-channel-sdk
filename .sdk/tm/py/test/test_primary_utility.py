...
from projectname_sdk import TelegramChannelSDK
from core.spec import TelegramChannelSpec
from core.result import TelegramChannelResult
from core.response import TelegramChannelResponse
from core.operation import TelegramChannelOperation
from core.error import TelegramChannelError
...
def _err_from_map(m):
    ...
    if msg == "":
        return None
    code = m.get("code", "")
    return TelegramChannelError(code, msg)
...
class TestPrimaryUtility:
    ...
    def test_make_error_no_throw(self):
        ...
        ctx.result = TelegramChannelResult({
            "ok": False,
            "resdata": {"id": "safe01"},
        })
    ...
    def test_feature_init_basic(self):
        init_client = TelegramChannelSDK.test(None, None)
        ...
        class TestInitFeature(TelegramChannelBaseFeature):
            def __init__(self):
                super().__init__()
                self.name = "initfeat"
                self.active = True
    ...
    def test_feature_init_inactive(self):
        ...
        class TestInitFeatureInactive(TelegramChannelBaseFeature):
            def __init__(self):
                super().__init__()
                self.name = "nofeat"
                self.active = False
    ...
    def test_fetcher_live(self):
        ...
        live_client = TelegramChannelSDK({
            "system": {
                "fetch": mock_fetch,
            },
        })
    ...
    def test_fetcher_blocked_test_mode(self):
        ...
        blocked_client = TelegramChannelSDK({
            "system": {
                "fetch": mock_fetch,
            },
        })
    ...
    def test_make_result_no_result(self):
        ...
        utility = client._utility
        ctx = _make_test_full_ctx(client, utility)
        ctx.spec = TelegramChannelSpec({"step": "start"})
        ctx.result = None
    ...
    def test_make_spec_basic(self):
        ...
        spec_client = TelegramChannelSDK.test(None, setup_opts)
    ...
    def test_operator_basic(self):
        ...
        def subject(entry):
            ...
            if not isinstance(in_val, dict):
                in_val = {}
            op = TelegramChannelOperation(in_val)
            return {
                "entity": op.entity,
                "name": op.name,
                "input": op.input,
    ...
    def test_prepare_auth_basic(self):
        ...
        auth_client = TelegramChannelSDK.test(None, setup_opts)
