...
module TelegramChannelUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = TelegramChannelResponse.new({})
    ...
    if fetch_err
      ...
      response = TelegramChannelResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = TelegramChannelResponse.new(fetched)
    else
