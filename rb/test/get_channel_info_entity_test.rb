...
class GetChannelInfoEntityTest < Minitest::Test
  ...
  def test_basic_flow
    setup = get_channel_info_basic_setup(nil)
    # Per-op sdk-test-control.json skip.
    _live = setup[:live] || false
    ["load"].each do |_op|
      _should_skip, _reason = Runner.is_control_skipped("entityOp", "get_channel_info." + _op, _live ? "live" : "unit")
      if _should_skip
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set TELEGRAMCHANNEL_TEST_GET_CHANNEL_INFO_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    get_channel_info_ref01_data = nil
    if get_channel_info_ref01_data_raw.length > 0
...
def get_channel_info_basic_setup(extra)
  Runner.load_env_local

  entity_data_file = File.join(__dir__, "..", "..", ".sdk", "test", "entity", "get_channel_info", "GetChannelInfoTestData.json")
  ...
  entid_env_raw = ENV["TELEGRAMCHANNEL_TEST_GET_CHANNEL_INFO_ENTID"]
  ...
  if env["TELEGRAMCHANNEL_TEST_LIVE"] == "TRUE"
    ...
    client = TelegramChannelSDK.new(Helpers.to_map(merged_opts))
  end
