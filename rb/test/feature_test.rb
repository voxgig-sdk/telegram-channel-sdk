...
module TelegramChannelFeatureHarness
  # True when this SDK was generated with the named feature.
  def self.has_feature?(name)
    f = TelegramChannelConfig.make_config["feature"]
  ...
  class Harness
    ...
    def initialize(features, server: nil, base: "http://api.test", headers: {})
      ...
      @utility = TelegramChannelUtility.new
      @utility.fetcher = server || TelegramChannelFeatureHarness.default_server
      ...
      features.each do |fspec|
        name = fspec["name"]
        next unless TelegramChannelFeatureHarness.has_feature?(name)
        f = TelegramChannelFeatures.make_feature(name)
    ...
    def op(opname: "load", entity: "widget", method: nil, path: nil, query: nil,
      ...
      method ||= TelegramChannelFeatureHarness.default_method(opname)
      ...
      begin
        fire(ctx, "PrePoint")
        raise ctx.out["point"] if ctx.out["point"].is_a?(TelegramChannelError)
        ...
        ctx.response = fetched.is_a?(Hash) ? TelegramChannelResponse.new(fetched) : nil
        ...
        raise err
      rescue TelegramChannelError => err
        ctx.ctrl.err = err
    ...
    def populate_result(ctx, fetched, fetch_err)
      result = TelegramChannelResult.new({})
      ctx.result = result
...
class FeatureTest < Minitest::Test
  H = TelegramChannelFeatureHarness
  ...
  def test_retry_retries_a_transport_error_tuple
    ...
    server, calls = H.recording_server { |n, _fd|
      n < 3 ? [nil, TelegramChannelError.new("boom", "boom")] : [H.make_response(200, { "ok" => true }), nil]
    }
  ...
  def test_retry_exhausted_transport_error_surfaces
    skip_unless_feature("retry")
    clock = H::Clock.new
    server, calls = H.recording_server { |_n, _fd| [nil, TelegramChannelError.new("boom", "boom")] }
    h = harness([fspec("retry",
      "retries" => 2, "minDelay" => 1, "jitter" => false, "sleep" => clock.sleeper)],
      server: server)
    res = h.op
