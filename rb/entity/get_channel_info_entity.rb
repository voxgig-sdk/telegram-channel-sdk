...
class GetChannelInfoEntity
  ...
  def make
    opts = @_entopts.dup
    GetChannelInfoEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = TelegramChannelHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [GetChannelInfo, Hash] the current GetChannelInfo data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = TelegramChannelHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of GetChannelInfo fields)
  def match_get
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = TelegramChannelHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
