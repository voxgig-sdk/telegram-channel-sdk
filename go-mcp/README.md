...
[MCP](https://modelcontextprotocol.io) server exposing the TelegramChannel SDK as
two agent tools — `telegram-channel_list` and `telegram-channel_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/telegram-channel-mcp)
make build

# 2. Provide credentials via the environment
export TELEGRAM_CHANNEL_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user telegram-channel \
  -- /absolute/path/to/telegram-channel-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./telegram-channel-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// telegram-channel_list: first page of records
{ "entity": "get_channel_info" }
{ "entity": "get_channel_info", "query": { } }

// telegram-channel_load: one record by id
{ "entity": "get_channel_info", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/telegram-channel-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user telegram-channel \
     -- "$PWD"/dist/*/telegram-channel-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `telegram-channel_list` and `telegram-channel_load` tools now appear
   in new sessions. Ask the agent to *"list get_channel_info using telegram-channel"*
   and it calls `telegram-channel_list` with `{"entity":"get_channel_info"}`.
...
```sh
export TELEGRAM_CHANNEL_APIKEY=sk_live_xxx            # API key
export TELEGRAM_CHANNEL_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `telegram-channel_list` tool
...
### Call the `telegram-channel_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `telegram-channel_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `telegram-channel_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `TELEGRAM_CHANNEL_APIKEY` | API key sent with every request. |
| `TELEGRAM_CHANNEL_BASE` | Optional override of the API base URL. |
...
```sh
./telegram-channel-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"telegram-channel_load","arguments":{"entity":"get_channel_info","query":{"id":1}}}}'
```
