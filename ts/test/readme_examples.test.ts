...
//     (\`new TelegramChannelSDK(...)\` / \`TelegramChannelSDK.test(...)\` become
//     \`TelegramChannelSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'TelegramChannelSDK'
...
const TEST_SEED = {"entity":{"get_channel_info":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new TelegramChannelSDK(...)\` and any
// \`TelegramChannelSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+TelegramChannelSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/TelegramChannelSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
