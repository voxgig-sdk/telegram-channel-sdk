...
`client.GetChannelInfo()` — each with a small set of operations (`load`)
...
```ts
try {
  const getchannelinfo = await client.GetChannelInfo().load({ id: 'example_id' })
...
// getchannelinfo is a bare entity populated with mock response data
console.log(getchannelinfo)
```
...
| `tester(testopts?, sdkopts?)` | `TelegramChannelSDK` | Create a test-mode client instance. |

#### Static methods

| Method | Returns | Description |
| --- | --- | --- |
| `TelegramChannelSDK.test(testopts?, sdkopts?)` | `TelegramChannelSDK` | Create a test-mode client. |
...
| `client` | `client(): TelegramChannelSDK` | Return the parent SDK client. |
...
telegram-channel/
├── src/
│   ├── TelegramChannelSDK.ts        # Main SDK class
